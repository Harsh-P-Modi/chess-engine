...
2. ~~Create `src/search/tt.cpp` - TT storage (~262K entries), probe/store methods~~
...
4. ~~Add `gen_captures()` function~~

### Phase 4: Move Ordering (`src/search/`)
1. ~~Add MVV-LVA scoring for captures~~
2. ~~Add killer/heuristic filtering~~
3. ~~Track best move from TT for ordering~~
4. ~~Pack Move into uint32_t (32 bits) for TTEntry = 16 bytes total~~

### Phase 5: Evaluation Improvements (`src/evaluation/`)
1. ~~Add piece-square tables for MG/EG~~
...
1. ~~Parse time controls from "go" command (wtime, btime, winc, binc, movetime)~~
